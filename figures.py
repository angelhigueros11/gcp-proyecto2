...
import numpy as np
from raytracerEngine import Point3
...
class DirectionalLight(object):
    def __init__(self, direction=Point3(0, -1, 0), intensity=1, color=WHITE):
...
class PointLight(object):
    ...
    def __init__(self, position=Point3(0, 0, 0), intensity=1, color=WHITE):
...
class Cube(object):
    ...
    def __init__(self, position, size, material=Material()):
        ...
        self.planes.append(
            Plane(np.add(position, Point3(halfSizeX, 0, 0)), Point3(1, 0, 0), material))
        self.planes.append(
            Plane(np.add(position, Point3(-halfSizeX, 0, 0)), Point3(-1, 0, 0), material))

        self.planes.append(
            Plane(np.add(position, Point3(0, halfSizeY, 0)), Point3(0, 1, 0), material))
        self.planes.append(
            Plane(np.add(position, Point3(0, -halfSizeY, 0)), Point3(0, -1, 0), material))

        self.planes.append(
            Plane(np.add(position, Point3(0, 0, halfSizeZ)), Point3(0, 0, 1), material))
        self.planes.append(
            Plane(np.add(position, Point3(0, 0, -halfSizeZ)), Point3(0, 0, -1), material))
    ...
    def ray_intersect(self, orig, dir):
        ...
        for plane in self.planes:
            ...
            if planeInter is not None and planeInter.point[0] >= self.boundsMin[0] and planeInter.point[0] <= self.boundsMax[0] and planeInter.point[1] >= self.boundsMin[1] and planeInter.point[1] <= self.boundsMax[1] and planeInter.point[2] >= self.boundsMin[2] and planeInter.point[2] <= self.boundsMax[2] and planeInter.distance < t:
